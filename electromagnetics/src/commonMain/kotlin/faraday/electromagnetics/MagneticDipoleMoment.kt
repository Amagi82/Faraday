...
/**
 *  The component of the magnetic moment that can be represented by an equivalent magnetic dipole: a magnetic north and south pole separated by a very small distance. The magnetic dipole component is sufficient for small enough magnets or for large enough distances. Higher order terms (such as the magnetic quadrupole moment) may be needed in addition to the dipole moment for extended objects.
 *
...
value class MagneticDipoleMoment(val joulesPerTesla: Double) : Units<MagneticDipoleMoment> {
    @InternalUnitApi
    override val rawValue: Double get() = joulesPerTesla

    @InternalUnitApi
    override fun create(value: Double) = MagneticDipoleMoment(value)
}
