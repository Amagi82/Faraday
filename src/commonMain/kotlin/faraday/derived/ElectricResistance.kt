 ...
 *
 * The resistance R of an object is defined as the ratio of voltage V across it to current I through
...
value class ElectricResistance(val ohms: Double) : Units<ElectricResistance> {
    ...
    override fun div(factor: Number) = ElectricResistance(ohms = ohms / factor.toDouble())
    override fun compareTo(other: ElectricResistance): Int = ohms.compareTo(other.ohms)

    operator fun times(current: ElectricCurrent): Voltage = current * this
    operator fun times(time: Time) = ElectricInductance(henrys = ohms * time.seconds)
    operator fun times(charge: ElectricCharge) = MagneticFlux(webers = ohms * charge.coulombs)
    operator fun div(frequency: Frequency) = ElectricInductance(henrys = ohms / frequency.hertz)

    fun asConductance() = ElectricConductance(siemens = 1 / ohms)
}
